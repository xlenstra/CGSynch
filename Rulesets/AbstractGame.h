...
//	virtual GameId getIdOrInsertIntoDatabase() = 0;
	...
	virtual bool determineDecidedSynchedValue() = 0;
...
AlternatingId determineAlternatingId(Game& game) {
	...
	for (const auto& leftPosition : game.getLeftOptions()) {
		leftOptions.insert(determineAlternatingId<Position, Game>(database.idToGame(leftPosition)));
	}
	for (const auto& rightPosition : game.getRightOptions()) {
		rightOptions.insert(determineAlternatingId<Position, Game>(database.idToGame(rightPosition)));
	}
...
template<isPosition Position, isGame<Position> Game>
SynchedId determineSynchedId(Game& game) {
	...
	for (size_t row = 0; row < synchedOptions.options.size(); ++row) {
		for (size_t column = 0; column < synchedOptions.options[row].size(); ++column) {
			auto id = determineSynchedId<Position, Game>(database.idToGame(synchedOptions.options[row][column]));
			synchedMatrix.matrix[row][column] = id;
		}
	}
	for (const auto& leftPosition : game.getLeftOptions()) {
		synchedMatrix.leftOptions.push_back(determineSynchedId<Position, Game>(database.idToGame(leftPosition)));
	}
	for (const auto& rightPosition : game.getRightOptions()) {
		synchedMatrix.rightOptions.push_back(determineSynchedId<Position, Game>(database.idToGame(rightPosition)));
	}
...
//template<isPosition Position>
//bool AbstractGame<Position>::determineDecidedSynchedValue() {
//	determineAlternatingId<Position>(*this);
//	CombinatorialGame& alternatingForm = CGDatabase::getInstance().idToGame(alternatingId);
//	if (!alternatingForm.isNumber()) return false;
//	double numberValue = alternatingForm.getNumberValue()->asDouble();
//	synchedId = SGDatabase::getInstance().getDecidedGameWithValueId(numberValue);
//	return true;
//}
